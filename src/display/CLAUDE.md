...
UI component layer built on Clay. Each frame, declares the full layout hierarchy using `CLAY()` macros. Owns the pane tree, tab list, visual line cache, theming/color resolution, cursor rendering, icons, status bar, and echo area display. Reads buffer state from `src/text/` but never mutates it.

## Files

- **layout.c / layout.h** — Top-level layout. `create_app_layout(state)` returns `Clay_RenderCommandArray`. Hierarchy: TabBar → TabContent (panes) → MessageArea. Called once per frame from `iterate.c`.
...
- **message.c / message.h** — Echo area at bottom of layout.
