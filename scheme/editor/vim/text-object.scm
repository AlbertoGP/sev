...
(register-text-object! 'word
  (lambda (kind count)
    (let ((len (buffer-length))
      ...
      (and (> len 0) (< pos len)
           (let* ((c (char-at pos))
                  (pred (cond ((vim-word-char? c) vim-word-char?)
                              ((vim-punctuation? c) vim-punctuation?)
                              (else vim-whitespace?))))
             (let ((start (let bwd ((s pos))
               ...
               (let ext ((e end) (n (- count 1)))
                 (if (<= n 0)
                     (if (eq? kind 'around)
                         (let ((ae (let tr ((p e))
                                     (if (and (< p len) (vim-whitespace? (char-at p)))
                                         (tr (+ p 1)) p))))
                           (if (> ae e)
                               (make-range start ae 'char)
                               (let ((as (let ld ((p start))
                                           (if (and (> p 0) (vim-whitespace? (char-at (- p 1))))
                                               (ld (- p 1)) p))))
                     ...
                     (let ((ws (let sk ((p e))
                                 (if (and (< p len) (vim-whitespace? (char-at p)))
                                     (sk (+ p 1)) p))))
                       (if (>= ws len) (ext ws 0)
                           (let ((cp (if (vim-word-char? (char-at ws))
                                         vim-word-char? vim-punctuation?)))
                             (ext (let sk ((p ws))
...
(register-text-object! 'WORD
  (lambda (kind count)
    (let ((len (buffer-length))
          (pos (point-get)))
      (and (> len 0) (< pos len)
           (let ((ws? (vim-whitespace? (char-at pos))))
             (let ((start (let bwd ((s pos))
                            (if (and (> s 0)
                                     (if ws? (vim-whitespace? (char-at (- s 1)))
                                         (not (vim-whitespace? (char-at (- s 1))))))
                                (bwd (- s 1)) s)))
                   (end (let fwd ((e pos))
                          (if (and (< e len)
                                   (if ws? (vim-whitespace? (char-at e))
                                       (not (vim-whitespace? (char-at e)))))
                              (fwd (+ e 1)) e))))
               ;; Extend for count
               (let ext ((e end) (n (- count 1)))
                 (if (<= n 0)
                     ...
                     (let ((ws (let sk ((p e))
                       ...
                       (if (>= ws len) (ext ws 0)
                           (ext (let sk ((p ws))
                                  (if (and (< p len) (not (vim-whitespace? (char-at p))))
                                      (sk (+ p 1)) p))
                                (- n 1))))))))))))
...
(defcommand (vim-in-text-object)
  "vim: inner text object\n"
  (vim-execute-text-object 'in))

(defcommand (vim-around-text-object)
  "vim: around text object"
  (vim-execute-text-object 'around))
...
(set-command-display-binding! 'vim-in-text-object     "i <obj>")
(set-command-display-binding! 'vim-around-text-object "a <obj>")

;; Bind i/a + char in pending and select maps
(for-each (lambda (ch)
  (let ((key (string ch)))
    (set-key! pending-map (string-append "i " key) 'vim-in-text-object)
    (set-key! pending-map (string-append "a " key) 'vim-around-text-object)
    (set-key! select-map (string-append "i " key) 'vim-in-text-object)
    (set-key! select-map (string-append "a " key) 'vim-around-text-object)))
  '(#\w #\W #\s #\p #\[ #\] #\( #\) #\b #\{ #\} #\B #\< #\> #\t #\" #\' #\`))
